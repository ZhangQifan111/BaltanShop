// 跑剩余 3 个系列 (scrape + 入库 + 下载图 + 更新 DB 本地路径)
// 已完成: ultraman / return-of-ultraman / ultraseven / ultraman-ace / ultraman-leo / ultramantaro / ultraman80 / ultrafight / corner813560
...
async function runOne(series) {
  ...
  if (scrapeResult.errors.length) console.log('  errors:', scrapeResult.errors.slice(0, 3));

  const refs = db.allSync('SELECT * FROM baltan_reference WHERE series = ?', [series]);
  if (!refs.length) { console.log('  no refs, skip'); return; }
...
(async () => {
  await db.getDb();
  const t0 = Date.now();
  for (const s of SERIES) await runOne(s);
  const t1 = Date.now();
